...
H = {
    ...
    "71w": "עמ' 71 (סעיף The Western Mediterranean Ridge)",
...
CITATIONS = {
    ...
    "(Huguen et al., 2006, עמ' 71, מערב)": [("Huguen", H["71w"])],
    ...
    "(Kopf et al., 2003, תקציר ואיור 1)": [("Kopf", "תקציר (פס' 1); איור 1a")],
...
# ציטוט נרטיבי: השנה נשארת בגוף המשפט, רק הלוקטור עובר להערה. אין כזה עוד -
# בכל העבודה שם המקור חי בהערת השוליים בלבד, וההפניה הנרטיבית היחידה שהייתה
# בפרק 5.1 בוטלה לטובת אחידות.
NARRATIVE = {}
...
TEXT_FIX = {
    ...
    # המונח décollement כבר מבואר בגוף בפרק 5.1, והחזרה עליו בכיתוב גם
    # מיותרת וגם נשברת בין שורות.
    "משמשות מישור ניתוק (décollement) בבסיס":
        "משמשות מישור ניתוק בבסיס",

    # Backstop היה המונח הלועזי היחיד שנשאר באנגלית לאורך כל הטקסט, בעוד
    # back-arc, forearc, décollement ו-en échelon כולם מבוארים פעם אחת
    # ואז נכתבים בעברית. "משענת" קולעת ל-mechanical buttress של Huguen.
    # הסדר כאן קובע: הביטויים הספציפיים, שדורשים גם התאמה דקדוקית, לפני
    # הכלל הכללי בסוף.
    "כ-Backstop שאפשר את הצטברות המנסרה":
        "כמשענת (backstop) שאפשרה את הצטברות המנסרה",
    "ניצב ה-Backstop ההלני": "ניצבת המשענת ההלנית",
    # אצל Huguen התואר מתאר את המשענת ("The flat and deeper backstop area"),
    # ו-"אזור המשענת השטוחה" היה מצמיד אותו למילה הלא נכונה.
    "ובאזור ה-Backstop השטוח היא משתרעת": "ובמשענת השטוחה היא משתרעת",
    "את ה-Backstop ואת אונות המלח שגלשו מעליו":
        "את המשענת ואת אונות המלח שגלשו מעליה",
    "מגע המנסרה-Backstop": "המגע בין המנסרה למשענת",
    "למגע שבין המנסרה ל-Backstop": "למגע שבין המנסרה למשענת",
    # באיור 3 המילה Backstop מודפסת על התמונה עצמה, ולכן הכיתוב מגשר אליה.
    "אזור ה-Backstop שמצפון לה": "אזור המשענת שמצפון לה (Backstop באיור)",
    "ה-Backstop": "המשענת",
    # הכותרת הייתה המקום האחרון שבו מונח לועזי עמד לבדו בלי ביאור עברי,
    # בעוד הגוף כותב "קדם-הקשת (forearc)".
    "קשת געשית ו-Forearc": "קשת געשית וקדם-קשת",
...
INSERTIONS = [
    ...
    (
        ...
        "(Huguen et al., 2006, עמ' 62). השקעים העמוקים מצויים רק בצידה "
        "הפנימי של המנסרה, ולא בחזיתה."
    ),
    (
        "הקינמטיקה של המערכת",
